"""
Container Stowage Optimizer - Genetic Algorithm + Simulated Annealing  v5
==========================================================================
"""
...
_n_locked_bay35 = len(_LOCKED_BAY3_BAY5_R3R6)
_n_locked_asymm = len(_LOCKED_ASYMMETRIC)
_n_locked_total = len(LOCK_20FT_ONLY_SLOTS)

print(f"""
==============================================================
  CONTAINER STOWAGE - GA + SA OPTIMIZER  v5-RANDOM
  40ft TIDAK BOLEH di BAWAH 20ft
  DISTRIBUSI SEIMBANG 20ft dan 40ft
  20ft BISA MENGISI SLOT KOSONG DI PAIR BAY
  v5: BAY 3&5 R3-R6 DIKUNCI HANYA 20ft (NO 40ft)
  SOLUSI AWAL: RANDOM VALID (tanpa greedy)
--------------------------------------------------------------
  ...
  Locked 20ft : {_n_locked_total:<3} slot ({_n_locked_bay35} Bay3&5-R3R6 + {_n_locked_asymm} asimetris)
==============================================================
""")
...
def read_containers(filename: str) -> List[Container]:
    ...
    print(f"  {len(containers)} containers | 20ft:{c20} | 40ft:{c40} | "
          ...
          f"weight {min(c.weight for c in containers):.1f}-{max(c.weight for c in containers):.1f}t")
    return containers
...
def col_type(s: np.ndarray, cmap: Dict[int,Container], b: int, r: int) -> str:
    ...
    if len(sizes) > 1:
        ...
        for ti in range(NT):
            ...
            if cid > 0:
                if cmap[cid].size == 1:
                    top_20_ti = max(top_20_ti, ti)
                else:
                    bot_40_ti = min(bot_40_ti, ti)
        if top_20_ti < bot_40_ti:
            return 'pair_split'
        else:
            return 'mixed'
...
def col_type_simple(s, cmap, b, r):
    ct = col_type(s, cmap, b, r)
    if ct in ('empty', '20ft', 'pair_split'):
        return ct
    return ct
...
def rebuild(col20, pair40, cmap, strict_40over20=True):
    ...
    for pidx in ALLOWED_40FT_PAIRS:
        ...
        for r in range(NR):
            ...
            low40 = NT
            for ti in range(NT):
                if s[pidx,r,ti] > 0 and cmap[s[pidx,r,ti]].size == 2:
                    low40 = ti; break
    ...
    all_20 = sorted(
        {cid for cids in col20.values() for cid in cids},
        key=lambda cid: -cmap[cid].weight
    )
...
# ============================================================================
# RANDOM VALID SOLUTION - pengganti greedy_solution()
# ============================================================================
def random_valid_solution(containers):
    """
    Membuat solusi awal secara ACAK namun tetap valid:
      1. Container 40ft diacak dan ditempatkan ke pair bay yang tersedia
         (hanya slot yang tidak locked, tidak bertumpuk, span 2 bay)
      2. Container 20ft diacak dan ditempatkan ke slot valid yang tersisa
         (boleh di pair bay maupun bay biasa, asal bukan di atas 40ft)
      3. rebuild() dipanggil untuk memastikan tidak ada 40ft di bawah 20ft
         dan tidak ada container melayang

    PERBEDAAN dari greedy_solution():
      - Tidak ada perhitungan rasio / alokasi proporsional
      - Tidak ada strategi interleave fore-aft
      - Urutan penempatan murni random (setelah shuffle)
      - Hasilnya lebih tidak seimbang, tapi tetap valid secara struktural
    """
    cmap = {c.id: c for c in containers}

    # Pisahkan dan acak urutan container
    c40_list = [c for c in containers if c.size == 2]
    c20_list = [c for c in containers if c.size == 1]
    random.shuffle(c40_list)
    random.shuffle(c20_list)

    n_20, n_40 = len(c20_list), len(c40_list)

    # Langkah 1: Tempatkan 40ft secara acak ke pair bay
    # Kumpulkan semua (pair_bay_idx, row) yang tersedia untuk 40ft
    pair_keys = list(PAIR_TIERS.keys())
    random.shuffle(pair_keys)  # acak urutan pair bay

    pair40: Dict = {}
    c40_remaining = list(c40_list)

    for key in pair_keys:
        if not c40_remaining:
            break
        pidx, r = key
        cap = len(PAIR_TIERS[key])  # kapasitas slot di pasangan ini

        # Ambil sejumlah container 40ft secara acak untuk pair ini
        # Jumlahnya acak antara 0 sampai min(cap, sisa container)
        max_here = min(cap, len(c40_remaining))
        n_here   = random.randint(0, max_here)  # acak: boleh kosong

        if n_here > 0:
            assigned = [c40_remaining.pop(0).id for _ in range(n_here)]
            pair40[key] = assigned

    # Jika masih ada 40ft yang belum ditempatkan, paksa masuk ke pair yang
    # masih punya kapasitas (agar tidak ada container yang hilang)
    if c40_remaining:
        for key in pair_keys:
            if not c40_remaining: break
            pidx, r = key
            cap      = len(PAIR_TIERS[key])
            existing = len(pair40.get(key, []))
            space    = cap - existing
            if space > 0:
                extra = [c40_remaining.pop(0).id for _ in range(min(space, len(c40_remaining)))]
                pair40.setdefault(key, []).extend(extra)

    # Langkah 2: Tentukan col20 secara acak
    # Kumpulkan semua (b, r) yang bisa diisi 20ft
    all_col_keys = list(COL_TIERS.keys())
    random.shuffle(all_col_keys)

    col20: Dict = {}
    c20_remaining = list(c20_list)

    for key in all_col_keys:
        if not c20_remaining: break
        b, r = key
        cap = len(COL_TIERS[key])

        # Di pair bay, kapasitas 20ft berkurang sesuai 40ft yang sudah dialokasikan
        if b in ALLOWED_40FT_PAIRS and not is_row_locked_for_pair(b, r):
            n40_here = len(pair40.get((b, r), []))
            cap = max(0, cap - n40_here)
        elif b in PAIR_SECOND_BAYS:
            first = b - 1
            n40_here = len(pair40.get((first, r), []))
            cap = max(0, cap - n40_here)

        if cap <= 0: continue

        # Ambil sejumlah acak container 20ft untuk kolom ini
    ...
    # Jika masih ada 20ft yang belum ditempatkan, paksa masuk ke kolom yang
    ...
    for b in range(NB):
        for r in range(NR):
            for t in range(NT):
                ...
                if cid <= 0: continue
                c = cmap[cid]
                if c.size == 1: placed_20 += 1
                elif cid not in seen40: seen40.add(cid); placed_40 += 1

    occ   = int(np.count_nonzero(s))
    total = placed_20 + placed_40
    ratio = placed_40 / total if total > 0 else 0
    unplaced_count = len(unplaced)

    print(f"  Random: {total}/{len(containers)} placed | 20ft:{placed_20} | 40ft:{placed_40} | "
          f"ratio_40={ratio:.2%} | {occ}/{TOTAL_SLOTS} slots | unplaced={unplaced_count}")
...
def fitness(s, cmap):
    ...
    lw = LIGHTSHIP['weight']
    tw = sum(t['weight'] for t in TANKS)
    base_w  = lw + tw
    ...
    v_penalty = _stab_penalty_teu(vcg_val, 9.0,
                                   a_lin=80.0, a_quad=60.0,
                                   tier2_teu=200.0, tier3_teu=400.0,
                                   thresh2=0.5, thresh3=1.0)
    l_penalty = _stab_penalty_teu(lcg_val, 2.0,
                                   a_lin=40.0, a_quad=20.0,
                                   tier2_teu=60.0, tier3_teu=150.0,
                                   thresh2=0.3, thresh3=1.0)
    t_penalty = _stab_penalty_teu(tcg_val, 0.0,
                                   a_lin=20.0, a_quad=15.0,
                                   tier2_teu=30.0, tier3_teu=80.0,
                                   thresh2=0.3, thresh3=1.0)
...
# ============================================================================
# OPTIMIZER - menggunakan random_valid_solution() sebagai inisialisasi
# ============================================================================
def optimize(containers, pop_size=50, generations=100, sa_steps=15, verbose=True):
    cmap={c.id:c for c in containers}
    n40=sum(1 for c in containers if c.size==2)
    n20=sum(1 for c in containers if c.size==1)
    print(f"\n{'='*65}")
    print(f"  GA+SA v5-RANDOM | pop={pop_size} gen={generations} sa={sa_steps}")
    print(f"  {n20}x20ft + {n40}x40ft | Locked: {_n_locked_total} slots")
    print(f"  Inisialisasi: RANDOM VALID (tanpa greedy)")
    print(f"{'='*65}")

    # Buat populasi awal dengan random_valid_solution
    print(f"\n  Membuat {pop_size} solusi awal (random valid)...")
    population = []
    for i in range(pop_size):
        s, _ = random_valid_solution(containers)
        # Tambahkan sedikit mutasi acak agar populasi lebih beragam
        if i > 0:
    ...
    # Tampilkan statistik fitness populasi awal
    ...
    for gen in range(generations):
        ...
        new_pop+=elite; new_scores+=[scores[ranked[0]],scores[ranked[1]]]
        ...
        while len(new_pop)<pop_size:
            ...
            cur=child.copy(); cf,*_=fitness(cur,cmap)
            for _ in range(sa_steps):
                r2=random.random()
                if r2<0.30:   cand=mutate_swap(cur,cmap)
                elif r2<0.55: cand=mutate_move(cur,cmap)
                elif r2<0.75: cand=mutate_reorder_col(cur,cmap)
                else:         cand=mutate_balance(cur,cmap)
                cand=fix_floating(cand,cmap)
                nf,*_=fitness(cand,cmap)
                delta=nf-cf
                if delta>0 or (temp>1 and random.random()<np.exp(delta/temp)):
                    cur=cand; cf=nf
            child=cur; cs=fitness(child,cmap)
        ...
        if verbose and (gen%10==0 or gen==generations-1):
            ...
            print(f"  Gen{gen:4d} | Fit(unified)={best_fit:>9.1f} | Fill={occ}/{TOTAL_SLOTS}({occ/TOTAL_SLOTS*100:.0f}%) | "
                  f"VCG={best_vcg:.3f}(D{abs(best_vcg-VCG_TARGET):.2f}) "
                  f"LCG={best_lcg:.3f}(D{abs(best_lcg-LCG_TARGET):.2f}) | "
                  f"40<20={v40u} Lock={vl} Bal={bal:.2f} | T={temp:.1f}")

    print(f"\n{'='*65}\n  SELESAI - Best fitness (unified) = {best_fit:.1f}\n{'='*65}")
    return best_s, best_fit

# ============================================================================
# VALIDATION & COG REPORT
# ============================================================================
def validate(s, cmap):
    c40_ids={c.id for c in cmap.values() if c.size==2}
            ...
            if top20>=0:
                for ti in ts:
                    ...
                    if cid>0 and cmap[cid].size==2:
                        err_40under.append(f"Bay{bi2n(b)} Row{ri2n(r)} T{ti2n(ti)} C{cid:03d}(40ft)<20ft")
            for ti in ts:
                cid=s[b,r,ti]
                if cid>0 and cmap[cid].size==2 and is_locked_20ft_only(b,r,ti):
                    err_lock.append(f"Bay{bi2n(b)} Row{ri2n(r)} T{ti2n(ti)} C{cid:03d} di locked slot")
            for i in range(1,len(ts)):
                ca_=s[b,r,ts[i]]; cb_=s[b,r,ts[i-1]]
                if ca_>0 and cb_==0: err_float.append(f"Bay{bi2n(b)} R{ri2n(r)} T{ti2n(ts[i])} melayang")
                if ca_>0 and cb_>0:
...
def cog_report(s, cmap):
    ...
    # Hitung n20/n40 dari grid s (container yang benar-benar ditempatkan)
    _placed20=set(); _placed40=set()
    for b in range(NB):
        for r in range(NR):
            for t in range(NT):
                cid=s[b,r,t]
                if cid<=0: continue
                if cmap[cid].size==1: _placed20.add(cid)
                else: _placed40.add(cid)
    n20=len(_placed20); n40=len(_placed40)
    ...
    print(f"  Fore: 20ft={f20} 40ft={f40} ratio={fr:.1f}%  |  Aft: 20ft={a20} 40ft={a40} ratio={ar:.1f}%")
    print(f"  -------------------------------------------------------")
...
# PRINT OVERVIEW
# ============================================================================
def print_overview(s: np.ndarray, cmap: Dict[int,Container]):
    print(f"\n{'='*72}")
    print(f"  BAY OVERVIEW")
    print(f"{'='*72}")
...
def _row_label(rn: int) -> str:
    if rn == 0: return "Row 0(C)"
    return f"Row {rn}(P)"

def _cell_str(s, cmap, b, r, t, is_second_bay, first_bay):
    valid = (b, r, t) in VALID_SLOTS
    if not valid: return "############"
    cid = s[b, r, t]
    if cid == 0: return "............"
    c = cmap[cid]
    ...
    if is_second_bay:
        if 0 <= first_bay < NB and s[first_bay, r, t] == cid: return "[REAR]"
        return "~orphan~"
    else:
        second = b + 1
        if second < NB and s[second, r, t] == cid: return f"{c.name}({c.weight:.0f}t)"
        return "~orphan~"
...
def print_all_bays(s, cmap):
    print(f"\n{'='*60}")
    print(f"  BAY LAYOUT DETAIL  ({NB} bays)")
    print(f"  Format: NAMA(BERATt) | ............=kosong | [REAR]=terblokir")
        ...
        for r in range(NR):
            for t in range(NT):
                ...
                pos.append({'Container_ID':c.name,'Size':f'{c.size*20}ft','Weight(ton)':c.weight,
                            ...
                            'MomVCG':round(c.weight*vcg,3),'MomLCG':round(c.weight*lcg,3),'Notes':notes})
    return pos
...
def export_excel(s, cmap, vcg, lcg, tcg, filename):
    ...
    ws['A1']="CONTAINER STOWAGE PLAN - GA+SA v5-RANDOM"
    ...
    sr=len(positions)+6
    ws.cell(row=sr,column=1,value="SUMMARY").font=Font(bold=True,size=12); sr+=1
    occ=sum(1 for (b,r,t) in VALID_SLOTS if s[b,r,t]!=0)
    ...
    for lbl,val,note in [
        ...
        ("40ft<20ft",v40u,"harus 0"),("Lock viol",vl,"harus 0"),("Balance",bal,"lebih kecil lebih baik"),
        ("Fore 20ft",f20,f"Bay1-{bi2n(mid-1)}"),("Fore 40ft",f40,f"ratio={f40/(f20+f40)*100:.1f}%" if (f20+f40)>0 else ""),
        ("Aft 20ft",a20,f"Bay{bi2n(mid)}-{bi2n(NB-1)}"),("Aft 40ft",a40,f"ratio={a40/(a20+a40)*100:.1f}%" if (a20+a40)>0 else ""),
    ]:
        ...
        if lbl in ("40ft<20ft","Lock viol") and val>0:
...
if __name__ == "__main__":
    ...
    best, best_fit = optimize(containers, pop_size=80, generations=50, sa_steps=50, verbose=True)
    ...
    export_excel(best, cmap, vcg, lcg, tcg, f"stowage_{base}_ga_sa_v5_random.xlsx")

    print(f"\n  Total waktu: {time.time()-t0:.1f} detik")

    print_unified_summary(
        "Program D - GA+SA v5-RANDOM (tanpa greedy)",
